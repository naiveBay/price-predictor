...
def runScript():
    SAMPLE_SIZE = 1;
    datasizes = [100, 200, 500, 800, 1200, 1903];
    binSize = 2;
    avgerror = [];
    for i in range(len(datasizes)):
        ...
        for j in range(SAMPLE_SIZE):
            print datasizes[i], j;
            err += crossValidate(binSize, datasizes[i])/SAMPLE_SIZE;
        avgerror.append(err);
        print err;
    print avgerror;
...
def splitItemSet(items,threshold):
    ...
    for i in range(len(items)):
        if random.random() < threshold: trainItems.append(items[i]);
...
## Selects a number of items from the set
def selectNumItems(items, num):
    if(num > len(items)): return items;
