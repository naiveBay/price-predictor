...
def generateDataMatrices(conn):
    ...
    items = getItems(complete=True,sold=True);
    for row in items:
        if(row[END_PRICE]):
...
def getItems(complete=True,sold="all",duration=0,genre=""):
    ...
    for row in db.execute(qString):  
        ...
        if duration==0:
            ...
            if ((complete == itemComplete) and (sold == itemSold) and (duration==itemDuration)  and row[START_PRICE]!="unknown"):
                items.insert(0,row)
...
def filterItems(items):
    ...
    print "# of items prefiltering: ",len(items)
    ...
    print "# of items after prefiltering: ",len(items)
    return items
